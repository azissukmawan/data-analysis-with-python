...
def create_rfm_df(df):
    rfm_df = df.groupby(by='customer_id', as_index=False).agg({
        'order_date': 'max', # mangambil tanggal order terakhir
        'order_id': 'nunique',
        'total_price': 'sum'
    })
...
# Pada contoh proyek ini, kita menampilkan informasi total order 
# dan revenue dalam bentuk metric() yang ditampilkan menggunakan layout columns().
...
# Pada proyek ini, kita akan menampilkan 5 produk paling laris dan paling sedikit terjual
...
sns.barplot(x='quantity_x', y='product_name', data=sum_order_items_df.sort_values(by='quantity_x', ascending=True).head(5), ax=ax[1], palette=colors)
...
# Nah, pada proyek ini, 
# kita akan menampilkan nilai average atau rata-rata dari ketiga parameter tersebut menggunakan widget metric()
...
with col2:
    ...
    st.metric(label='Average Recency', value=avg_frequency)
